...
Three data categories are retrieved from the ENTSO-E Transparency Platform and summarized in Table~\ref{tab:data_sources}:
...
Figures~\ref{fig:price_history}--\ref{fig:residual_load} summarize the corrected development sample. Figure~\ref{fig:price_history} shows the long-run price path, Figure~\ref{fig:price_distribution} shows the asymmetric distribution with negative observations and a pronounced upper tail, Figure~\ref{fig:seasonality} documents strong intraday and weekday structure, and Figure~\ref{fig:residual_load} illustrates the positive but nonlinear association between forecast residual load and price. These descriptive patterns motivate both the lag/calendar predictors and the inclusion of system fundamentals; they are not used as holdout evidence.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\linewidth]{fig2_price_distribution.png}
\caption{Distribution of DE-LU day-ahead hourly prices in the corrected development sample. The distribution includes negative prices and a pronounced upper tail.}
\label{fig:price_distribution}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\linewidth]{fig3_seasonality_hour_weekday.png}
\caption{Intraday and weekly seasonal structure of DE-LU day-ahead prices in the corrected development sample.}
...
The same regulation requires day-ahead wind and solar generation forecasts no later than 18:00 Brussels time on D-1, with subsequent intraday updates \citep{eureg543}. ENTSO-E's Transparency Platform FAQ identifies process type A01 as the day-ahead wind/solar forecast and explains that different forecast versions are submitted as distinct documents \citep{entsoefaq2024}. The A01 label therefore identifies forecast type but does not, by itself, establish the exact publication time or version available at 11:45. Because the historical interface used here does not reconstruct the complete document-version history, strict point-in-time admissibility of the stored renewable forecast at the simulated cutoff cannot be proven.

Accordingly, the \emph{Full} information set contains load plus aggregate renewables, residual load and renewable share, while the \emph{Tier-1} information set removes the three renewable-derived predictors. Tier-2 denotes variables whose exact 11:45 historical vintage is unresolved. It does not mean that leakage has been proven. The Full specification should therefore be interpreted as an information-rich benchmark whose strict real-time admissibility at 11:45 is not established, while Tier-1 provides the more conservative point-in-time robustness specification.
...
Four chronological outer validation blocks are defined within development, as shown in Table~\ref{tab:splits}:
