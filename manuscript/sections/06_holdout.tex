...
\begin{table}[htbp]
\centering
\caption{Frozen holdout strategy results at eta=0.85 and c=\EUR{10}.}
...
\[
\Delta_{forecast}=S2-S1=\EUR{1,437.60}.
\]
...
The Full uncertainty-aware strategy earns \EUR{19,130.01} compared with \EUR{20,002.02} for S2, so
\[
\Delta_{uncertainty}=-\EUR{872.01}.
\]
...
The substantive empirical result is therefore the value of the additional abstention. The filter forgoes \EUR{952.46} of profitable opportunities and avoids \EUR{80.44} of losses, reconciling to the reported aggregate gap up to rounding.

The reduction in aggregate profit is accompanied by substantial downside improvement. The hit rate rises from 91.9\% to 99.4\%, the worst day improves from -\EUR{17.55} to -\EUR{0.83}, and maximum drawdown improves from approximately -\EUR{22.61} to -\EUR{0.83}.
...
Applying the uncertainty filter to Tier-1 produces S5, which trades on only 117 days with a 99.1\% hit rate and earns \EUR{14,721.23}. It forgoes \EUR{4,654.82} of profitable outcomes while avoiding only \EUR{157.64} of losses relative to S4.

\subsection{Oracle value capture}

The ex-post oracle earns \EUR{21,243.10}. The corresponding value-capture ratios are 87.4\% for S1, 94.2\% for S2, 90.1\% for S3, 90.5\% for S4 and 69.3\% for S5.
...
The holdout contains 69 negative-price days, 47 days with a price above \EUR{200}/MWh and three days above \EUR{500}/MWh. On negative-price days,
\[
\Delta_{forecast}=\EUR{153.01},
\]
while on the remaining days it is \EUR{1,284.58}. On days above \EUR{200}/MWh, the contrast remains positive at \EUR{262.60}. The three days above \EUR{500}/MWh yield identical cumulative P\&L across active strategies, so no separate inference is drawn from that very small subgroup.
...
To compare XGBoost with simple rules that exploit recurring intraday structure, a weekday-aware naive benchmark and a trailing seven-day hourly mean profile were evaluated post hoc. The trailing profile is evaluated on all 212 holdout days by using late-December 2025 realized price profiles to initialize the first seven January forecasts. Thus Table~\ref{tab:posthoc_benchmarks} uses the same 5,087 hourly observations and 212 delivery days as the frozen holdout.

\begin{table}[htbp]
\centering
\caption{Post-hoc simple-benchmark comparison on the full 212-day holdout sample.}
...
\midrule
Lag-24 & 29.33 & 46.86 & 18,564.42 & 87.4\% \\
Weekday-aware naive & 30.00 & 47.69 & 18,374.95 & 86.5\% \\
Trailing 7-day mean profile & 29.56 & 43.73 & 19,722.29 & 92.8\% \\
XGBoost Tier-1 & 24.36 & 38.41 & 19,218.41 & 90.5\% \\
XGBoost Full & \textbf{17.51} & \textbf{29.87} & \textbf{20,002.02} & \textbf{94.2\%} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

Full XGBoost remains the most accurate model by a wide margin, but the economic comparison is considerably tighter. Its net P\&L exceeds the seven-day trailing-profile rule by \EUR{279.73}. Relative to lag-24, the trailing profile captures 80.5\% of Full's \EUR{1,437.60} incremental gain. Tier-1 trails the trailing profile by \EUR{503.88}. Across an exploratory 3-, 7-, 10-, 14-, 21- and 28-day profile family, Full's margin ranges from approximately \EUR{279.73} to \EUR{492.44}; the seven-day specification is the strongest of those simple benchmarks against Full. This shows that a large share of the economic value can be captured by a simple rule that learns the recurring hourly price shape, even though that rule has substantially worse point-forecast MAE than Full XGBoost.
...
S3--S2 & 212 & -872.01 & [-1,465.17, -358.72] & 16 & 34 \\
S4--S2 & 212 & -783.61 & [-1,270.35, -340.96] & 49 & 97 \\
S5--S3 & 212 & -4,408.77 & [-6,229.93, -2,728.84] & 29 & 91 \\
S2--Trailing 7-day & 212 & 279.73 & [-89.49, 640.59] & 87 & 61 \\
S4--Trailing 7-day & 212 & -503.88 & [-1,044.44, -53.68] & 62 & 73 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

The frozen S2--S1 contrast remains positive under this supplementary paired analysis. By contrast, the Full XGBoost advantage over the post-hoc trailing-profile rule is much smaller and its block-bootstrap interval includes zero. Tier-1 is lower than the trailing profile by \EUR{503.88}, and that post-hoc block-bootstrap interval excludes zero. The monthly S2--S1 decomposition is also uneven: January, February and March contribute \EUR{340.52}, \EUR{367.45} and \EUR{300.87} respectively, or about 70.2\% of the full \EUR{1,437.60} contrast; April--June contribute \EUR{433.70} in total; July is approximately flat at -\EUR{4.94}.

\paragraph{Threshold-frontier diagnostic.}
Because the frozen pooled-residual uncertainty rule is an abstention threshold, a post-hoc fixed-threshold frontier helps interpret whether its economic behavior is distinctive. A constant threshold of approximately \EUR{42.50} produces 160 trades, the same count as S3, and overlaps with S3 on 158 of those trading days; it earns \EUR{19,079.26} compared with \EUR{19,130.01} for S3. A lower fixed threshold of \EUR{20} trades on 187 days, earns \EUR{19,919.24}, achieves a 96.8\% hit rate and has a worst day of -\EUR{8.38}. These exploratory results reinforce that the frozen S3 rule mainly implements a time-varying participation hurdle rather than extracting a materially different intraday price pair.
