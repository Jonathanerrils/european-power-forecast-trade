...
The analysis proceeds through four layers: point forecasting, forecast uncertainty, economic decision-making and tail-risk evaluation. Figure~\ref{fig:workflow} summarizes this forecast-to-trade sequence. Development choices are completed before the 2026 holdout is opened.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig5_forecast_to_trade_workflow.png}
...
The selected final configurations are reported in Table~\ref{tab:final_hyperparams}:
...
Model files, predictor lists and manifests were checksum-verified before holdout evaluation. Both selected final models lie on at least one boundary of the frozen tuning grid: depth 3 is the minimum candidate for both models, while the Full model also selects the maximum tested learning rate of 0.10. These boundary selections are reported as a limitation of the finite search space rather than used to expand the grid after holdout exposure.
...
The economic layer is a stylized single-cycle daily storage problem. For each delivery day, all pairs \((i,j)\) satisfying \(i<j\) are candidates. The charge quantity is normalized to 1 MWh in one hourly interval, with round-trip efficiency \(\eta_{rt}\) applied to the discharge leg. Because prices are measured in EUR/MWh, reported daily and cumulative P\&L therefore have units of euros per normalized 1-MWh daily opportunity. They are not returns on a fully specified battery asset or invested capital.
...
\end{equation}
Because the residual offsets are delivery-day specific but common to all hours within day \(D\),
\[
L_{D,h}=\widehat P_{D,h}+r_D(0.10),\qquad
U_{D,h}=\widehat P_{D,h}+r_D(0.90).
\]
Substitution gives
\begin{align}
Score_U(i,j)
&=\eta_{rt}\widehat P_j-\widehat P_i-C
   +\eta_{rt}r_D(0.10)-r_D(0.90)\\
&=Score_P(i,j)-\tau_D,
\end{align}
where
\begin{equation}
\tau_D=r_D(0.90)-\eta_{rt}r_D(0.10).
\end{equation}
Thus \(\tau_D\) is constant across all candidate pairs within a delivery day. The uncertainty-aware rule therefore preserves the point strategy's ranking of intraday pairs exactly: whenever it trades, it selects the same maximizing pair as the corresponding point strategy; its only additional action is to abstain when the point strategy's best score does not exceed \(\tau_D\). Under the frozen pooled-residual construction, the uncertainty layer is therefore a day-varying abstention threshold. The marginal bounds remain useful as a conservative decision filter, but they are not a jointly calibrated interval for the price spread.
...
The primary holdout sign-consistency criterion concerns only \(\Delta_{forecast}\). The frozen rule classifies the result as full sign consistency when the primary cell and all nine sensitivity cells are positive, partial sign consistency when the primary cell is positive but fewer than nine cells are positive, and failure when the primary cell is non-positive. These nine cells are mechanically related sensitivity scenarios, not independent replications: the degradation cost enters as a common within-day pair cost and therefore primarily changes net value and abstention, while efficiency can also affect pair ranking.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig6_chronological_evaluation_design.png}
...
\label{sec:posthoc_methods}

After the January--July 2026 holdout had been exposed, additional diagnostics were introduced to probe the strength and interpretation of the frozen results. These analyses are explicitly supplementary and do not redefine the original frozen sign-consistency rule.

First, two stronger simple economic benchmarks were added. A weekday-aware naive forecast uses lag-24 on Tuesday--Friday and lag-168 on Saturday--Monday. A trailing seven-completed-day hourly mean profile forecasts each local delivery hour by the mean realized price at that same local hour over the preceding seven completed delivery days. The profile is evaluated on all 212 holdout days: the first seven holdout days are initialized from realized price profiles in late December 2025, which were already published before the corresponding 2026 D-1 decision origins. The seven-day rule is retained as the originally reported post-hoc profile benchmark. Because that window was itself chosen after holdout exposure, an additional exploratory sensitivity family of 3, 10, 14, 21 and 28 days is reported rather than treating seven days as pre-specified.

Second, paired daily P\&L differences are summarized using circular moving-block bootstrap intervals with seven-day blocks, 20,000 resamples and fixed seed 20260920. These intervals are post-hoc descriptive inference and are not part of the frozen success criterion. A fixed-threshold frontier is also reported post hoc to interpret the pooled-residual uncertainty rule against simpler constant participation hurdles.

Third, holdout interval calibration is reported for Full and Tier-1 using empirical coverage of the nominal central 80\% interval, lower- and upper-tail miss rates, mean interval width and the same Winkler score used during development. Monthly and local-hour diagnostics are treated as exploratory because they were not pre-specified.

All calculations are reproduced by \texttt{scripts/posthoc\_robustness\_v1.py}; derived outputs are stored under \texttt{outputs/posthoc/robustness\_v1/}.
\subsection{Holdout updating and frozen protocol}

Figure~\ref{fig:timeline} summarizes the chronological separation between development, protocol freeze and the first-look holdout. At the start of 2026, uncertainty history consists of the development out-of-sample residuals, not in-sample residuals from the final refitted models. For each holdout day \(D\), the uncertainty snapshot is formed using eligible development residuals plus residuals from already revealed holdout days. The day-\(D\) residual is appended only after prices for \(D\) are revealed and can therefore affect \(D+1\) onward.

The holdout boundary, final model-refit rule, uncertainty specification, strategy definitions, primary parameter cell, nine-cell grid, common-day rule, VaR/ES method and mechanical sign-consistency rule were all frozen before the first 2026 outcome metric was inspected. The complete holdout orchestration was rehearsed on non-holdout data before exposure. If a bug had been discovered after exposure, the protocol required preservation of the original result and explicit labeling of any corrective analysis rather than silent replacement.
